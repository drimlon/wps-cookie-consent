...
class Settings_API implements Actions {
     ...
     */
    const REST_NAMESPACE = 'wps-cookie-consent/v1';
    ...
    private function are_cookie_tables_changed( array $prev_settings, array $new_settings ): bool {
        ...
        $prev_cookie_table = $prev_settings['wps_options']['cookie_table'];
        $new_cookie_table = $new_settings['wps_options']['cookie_table'];
    ...
    public function update_settings( WP_REST_Request $request ) {
        ...
        // Validate nonce
        if ( ! wp_verify_nonce( $nonce, 'wps_cookie_consent_rest' ) ) {
            $this->logger->error( 'Updating settings failed due to invalid nonce' );

            return new WP_Error(
                'invalid_nonce',
                __( 'Invalid nonce.', 'wps-cookie-consent' ),
                array( 'status' => 403 )
            );
        }
