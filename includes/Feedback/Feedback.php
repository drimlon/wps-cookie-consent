...
class Feedback implements Actions {
    ...
    public function enqueue_feedback_script( string $hook_suffix ): void {
        ...
        wp_localize_script(
            'cookie-consent-feedback-admin-script',
            'wpsCCFeedbackDetails',
            array(
                'api' => array(
                    'route' => 'wps-cookie-consent/v1/feedback',
                    'nonce' => wp_create_nonce( 'wps_cookie_consent_feedback_rest' ),
                ),
            )
        );
    }
    ...
    public function send_plugin_feedback( WP_REST_Request $request ) {
        ...
        // Validate nonce
        if ( ! wp_verify_nonce( $nonce, 'wps_cookie_consent_feedback_rest' ) ) {
            return new WP_Error(
                'invalid_nonce',
