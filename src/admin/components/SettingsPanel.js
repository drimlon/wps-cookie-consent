...
function SettingsPanel() {
  ...
  const fetchSettings = async () => {
    const { route } = wpsCCAdminDetails.api;
  ...
  const validateState = () => {
    ...
    const blockedScripts = state?.wps_options?.blocked_scripts;
  ...
  const saveSettings = async (data) => {
    const { route, nonce } = wpsCCAdminDetails.api;
    ...
    try {
      ...
      if ('success' in response && response.success) {
        appendNotice({
          message: __('Settings saved successfully.', 'wps-cookie-consent'),
          status: 'success',
        });
      } else {
        appendNotice({
          message: __('Could not save settings.', 'wps-cookie-consent'),
          status: 'error',
        });
      }
    } catch (error) {
      if ('code' in error && error.code === 'invalid_nonce') {
        appendNotice({
          message: __('Could not pass security check.', 'wps-cookie-consent'),
          status: 'error',
        });
      } else {
        appendNotice({
  ...
  const resetSettings = async () => {
    ...
    try {
      ...
      if ('success' in response && response.success) {
        appendNotice({
          message: __('Settings reset successfully.', 'wps-cookie-consent'),
          status: 'success',
        });
      } else {
        appendNotice({
          message: __('Could not reset settings.', 'wps-cookie-consent'),
          status: 'error',
        });
      }
    } catch (error) {
  ...
  const clearRecords = async () => {
    const { consents_route: route, nonce } = wpsCCAdminDetails.api;
    ...
    try {
      ...
      if ('success' in response && response.success) {
        appendNotice({
          message: __('All consent records were cleared successfully.', 'wps-cookie-consent'),
          status: 'success',
        });
      } else {
        appendNotice({
          message: __('Could not clear records.', 'wps-cookie-consent'),
          status: 'error',
        });
      }
    } catch (error) {
  ...
  const ccSettings = useMemo(() => {
    ...
    const necessaryTable = settings.wps_options.cookie_table.necessary;
    const analyticsTable = settings.wps_options.cookie_table.analytics;
    const targetingTable = settings.wps_options.cookie_table.targeting;
    const preferencesTable = settings.wps_options.cookie_table.preferences;

    const primaryBtnRole = settings.wps_options.primary_btn_role;
    const secondaryBtnRole = settings.wps_options.secondary_btn_role;
  ...
  const eraseConsentCookies = () => {
    const cookieName = window.wpsCookieConsent.getConfig('cookie_name');
    window.wpsCookieConsent.eraseCookies(cookieName);
  };

  const resetPreview = (customSettings = {}) => {
    // Re-create the style element
    const styleElement = document.querySelector('#wps-cc-styles');

    if (styleElement) {
      let css = '';

      if (ccSettings.wps_options.font.slug !== 'default') {
        css += `--cc-font-family: ${ccSettings.wps_options.font.family};\n`;
      }

      Object.keys(ccSettings.wps_options.colors).forEach((key) => {
        const value = ccSettings.wps_options.colors[key];
        css += `--cc-${key}: ${value};\n`;
      });
    ...
    const config = deepCopy({
      ...ccSettings,
      ...customSettings,
      onAccept: () => window.wpsFloatingButton.show(),
      onChange: () => window.wpsFloatingButton.show(),
    });

    if (ccSettings.wps_options.hide_empty_categories) {
    ...
    window.wpsCookieConsent = window.initCookieConsent();
    window.wpsCookieConsent.run(config);

    // Re-initialize floating button
    window.wpsFloatingButton.init(ccSettings.wps_options.floating_button);
  };
  ...
  const previewSettingsModal = () => {
    ...
    // Show settings modal
    window.wpsCookieConsent.showSettings();
  };

  const previewFloatingButton = async () => {
    resetPreview();

    window.wpsFloatingButton.hide();
  ...
  const downloadJsonFile = (data) => {
    ...
    const filename = `wps-cookie-consent-settings_${currentTimestamp}.json`;
  ...
  const exportSettings = async () => {
    try {
      ...
      appendNotice({
        message: __('Could not export settings.', 'wps-cookie-consent'),
        status: 'error',
      });
    }
  };
  ...
  const importSettings = async (files) => {
    try {
      if (files.length === 0) {
        throw new Error(__('No files selected', 'wps-cookie-consent'));
      }
  ...
  const exportConsentRecords = async () => {
    const { export_route: route, nonce } = wpsCCAdminDetails.api;
    ...
    if (response.status !== 200) {
      ...
      appendNotice({
        message: __('Could not export consent records.', 'wps-cookie-consent'),
        status: 'error',
      });
  ...
  useEffect(() => {
    ...
    if (shouldShowNotice && !noticeExists) {
      appendNotice({
        message: __('Empty categories are hidden, and no cookies are listed, which might lead to issues with Google Consent Mode.', 'wps-cookie-consent'),
        status: 'warning',
        id: noticeId,
      });
      ...
      <Panel>
        <TabPanel
          ...
          tabs={[
            {
              name: 'general',
              title: __('General', 'wps-cookie-consent'),
              className: 'tab-general',
              Component: GeneralTab,
            },
            {
              name: 'cookies',
              title: __('Cookies', 'wps-cookie-consent'),
              className: 'tab-cookies-list',
              Component: CookiesTab,
            },
            {
              name: 'translations',
              title: __('Translations', 'wps-cookie-consent'),
              className: 'tab-translations',
              Component: TranslationsTab,
            },
            {
              name: 'modals',
              title: __('Modals', 'wps-cookie-consent'),
              className: 'tab-modals',
              Component: ModalsTab,
            },
            {
              name: 'floating-button',
              title: __('Floating Button', 'wps-cookie-consent'),
              className: 'tab-floating-button',
              Component: FloatingButtonTab,
            },
            {
              name: 'consent-mode',
              title: __('Consent Mode', 'wps-cookie-consent'),
              className: 'tab-consent-mode',
              Component: ConsentModeTab,
            },
            {
              name: 'blocked-scripts',
              title: __('Blocked Scripts', 'wps-cookie-consent'),
              className: 'tab-blocked-scripts',
              Component: BlockedScriptsTab,
            },
            {
              name: 'consent-records',
              title: __('Consent Records', 'wps-cookie-consent'),
              className: 'tab-consent-records',
              Component: ConsentRecordsTab,
            },
            {
              name: 'logs',
              title: __('Logs', 'wps-cookie-consent'),
              className: 'tab-logs',
              Component: LogsTab,
            },
            {
              name: 'about',
              title: __('About', 'wps-cookie-consent'),
              className: 'tab-about',
              Component: AboutTab,
            },
          ]}
