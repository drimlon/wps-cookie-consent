...
function CookiesTable(props) {
  ...
  const cookies = useMemo(
    () => {
      if (!state.wps_options.cookie_table) {
        return [];
      }

      return state.wps_options.cookie_table[category];
    },
    [state, category],
  );
      ...
      <FlexItem>
        {hasCookies ? (
          <Table width="100%">
            <Header>
              <Column>
                {__('Name', 'wps-cookie-consent')}
              </Column>
              <Column>
                {__('Domain', 'wps-cookie-consent')}
              </Column>
              <Column>
                {__('Expiration', 'wps-cookie-consent')}
              </Column>
              <Column>
                {__('Path', 'wps-cookie-consent')}
              </Column>
              <Column>
                {__('Description', 'wps-cookie-consent')}
              </Column>
              ...
              <Row>
                ...
                <Column>
                  <TextareaControl
                    ...
                    placeholder={__('This is an example cookie.', 'wps-cookie-consent')}
          ...
          <p>
            {__('No cookies in this category.', 'wps-cookie-consent')}
          </p>
        )}
      </FlexItem>
      <FlexItem>
        <Button
          ...
          isPrimary
        >
          {__('New Cookie', 'wps-cookie-consent')}
        </Button>
      </FlexItem>
    </Flex>
  );
